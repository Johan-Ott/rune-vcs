...
pub enum TeamsCmd {
    ...
    Show { name: String },
    /// Add member to team
    AddMember { team: String, member: String },
    /// Remove member from team
    RemoveMember { team: String, member: String },
    /// Set team lead
    SetLead { team: String, lead: String },
    /// Remove a team
    Remove { name: String },
    /// Show team workload
    Workload { 
...
pub enum AnalyticsCmd {
    ...
    Blocked,
    /// Show cycle health metrics
    CycleHealth {
        #[arg(long)]
        cycle: Option<String>,
    },
    /// Show team performance
    TeamPerformance { team: String },
    /// Show dependency tree for an item
    Dependencies { id: String },
    /// Show bottlenecks in the project
    Bottlenecks,
...
pub fn execute_plan_command(args: PlanArgs) -> Result<()> {
    ...
    match args.command {
        ...
        PlanCmd::Teams { cmd } => {
            handle_teams_command(cmd, &root)?;
        }
        
        PlanCmd::Analytics { cmd } => {
            handle_analytics_command(cmd, &root)?;
        }
    }
    Ok(())
}

fn handle_teams_command(cmd: TeamsCmd, root: &std::path::Path) -> Result<()> {
    ...
    // Load existing teams
    let mut teams: HashMap<String, serde_json::Value> = if teams_file.exists() {
    ...
    match cmd {
        TeamsCmd::List => {
            if teams.is_empty() {
                println!("No teams configured yet. Use 'rune plan teams create <name>' to add teams.");
            } else {
                println!("🏢 Teams\n");
                for (name, info) in &teams {
                    let members = info.get("members").and_then(|m| m.as_array()).map(|arr| arr.len()).unwrap_or(0);
                    let lead = info.get("lead").and_then(|l| l.as_str()).unwrap_or("No lead");
                    let description = info.get("description").and_then(|d| d.as_str()).unwrap_or("");
                    
                    println!("📋 {} ({} members)", name.cyan().bold(), members);
                    println!("   Lead: {}", lead);
        ...
        TeamsCmd::Create { name, description, members } => {
            let mut team_info = serde_json::Map::new();
            team_info.insert("description".to_string(), serde_json::Value::String(description.unwrap_or_else(|| format!("{} team", name))));
            team_info.insert("created".to_string(), serde_json::Value::String(chrono::Utc::now().to_rfc3339()));
            
            let member_list: Vec<serde_json::Value> = if let Some(members_str) = members {
                members_str.split(',').map(|m| serde_json::Value::String(m.trim().to_string())).collect()
            } else {
                vec![]
            };
            team_info.insert("members".to_string(), serde_json::Value::Array(member_list));
            
            teams.insert(name.clone(), serde_json::Value::Object(team_info));
            
            // Save teams
            fs::create_dir_all(teams_file.parent().unwrap())?;
            fs::write(&teams_file, serde_json::to_string_pretty(&teams)?)?;
            
            println!("✓ Created team: {}", name.cyan().bold());
        }
        
        TeamsCmd::Show { name } => {
            if let Some(info) = teams.get(&name) {
                let empty_members = vec![];
                let members = info.get("members").and_then(|m| m.as_array()).unwrap_or(&empty_members);
                let lead = info.get("lead").and_then(|l| l.as_str()).unwrap_or("No lead assigned");
                let description = info.get("description").and_then(|d| d.as_str()).unwrap_or("");
                ...
                let store = PlanStore::new(&root);
                let plans = store.load_all()?;
                let team_plans: Vec<_> = plans.iter().filter(|p| {
                    p.owners.iter().any(|owner| members.iter().any(|m| m.as_str() == Some(owner)))
                }).collect();
                
                if !team_plans.is_empty() {
                    println!("\nCurrent workload:");
                    for plan in team_plans {
                        println!("  {} - {} ({})", plan.id, plan.title, plan.status.as_str());
                    }
                }
            } else {
                println!("❌ Team '{}' not found", name);
            }
        }
        
        TeamsCmd::AddMember { team, member } => {
            if let Some(team_info) = teams.get_mut(&team) {
                if let Some(members_array) = team_info.get_mut("members").and_then(|m| m.as_array_mut()) {
                    let member_value = serde_json::Value::String(member.clone());
                    if !members_array.contains(&member_value) {
                        members_array.push(member_value);
                        fs::write(&teams_file, serde_json::to_string_pretty(&teams)?)?;
                        println!("✓ Added {} to team {}", member.green(), team.cyan());
                    } else {
                        println!("ℹ {} is already a member of {}", member, team);
                    }
                }
            } else {
        ...
        TeamsCmd::RemoveMember { team, member } => {
            if let Some(team_info) = teams.get_mut(&team) {
                if let Some(members_array) = team_info.get_mut("members").and_then(|m| m.as_array_mut()) {
                    let member_value = serde_json::Value::String(member.clone());
                    if let Some(pos) = members_array.iter().position(|m| m == &member_value) {
                        members_array.remove(pos);
                        fs::write(&teams_file, serde_json::to_string_pretty(&teams)?)?;
                        println!("✓ Removed {} from team {}", member.red(), team.cyan());
                    } else {
                        println!("ℹ {} is not a member of {}", member, team);
                    }
                }
            } else {
                println!("❌ Team '{}' not found", team);
        ...
        TeamsCmd::SetLead { team, lead } => {
            if let Some(team_info) = teams.get_mut(&team) {
                team_info.as_object_mut().unwrap().insert("lead".to_string(), serde_json::Value::String(lead.clone()));
                fs::write(&teams_file, serde_json::to_string_pretty(&teams)?)?;
                println!("✓ Set {} as lead of team {}", lead.green(), team.cyan());
            } else {
                println!("❌ Team '{}' not found", team);
            }
        }
        
        TeamsCmd::Remove { name } => {
            if teams.remove(&name).is_some() {
                fs::write(&teams_file, serde_json::to_string_pretty(&teams)?)?;
                println!("✓ Removed team: {}", name.red());
            } else {
                println!("❌ Team '{}' not found", name);
            }
        }
        
        TeamsCmd::Workload { team } => {
            let store = PlanStore::new(&root);
            let plans = store.load_all()?;
            
            if let Some(team_name) = team {
                if let Some(team_info) = teams.get(&team_name) {
                    let empty_members = vec![];
                    let members = team_info.get("members").and_then(|m| m.as_array()).unwrap_or(&empty_members);
                    let team_plans: Vec<_> = plans.iter().filter(|p| {
                        p.owners.iter().any(|owner| members.iter().any(|m| m.as_str() == Some(owner)))
                    }).collect();
                    
                    println!("📊 Workload for team: {}\n", team_name.cyan().bold());
                    
                    let total_plans = team_plans.len();
                    let completed = team_plans.iter().filter(|p| p.status == PlanStatus::Done).count();
                    let in_progress = team_plans.iter().filter(|p| p.status == PlanStatus::InProgress).count();
                    let planned = team_plans.iter().filter(|p| p.status == PlanStatus::Planned).count();
                    
                    println!("Total items: {}", total_plans);
                    println!("✅ Completed: {} ({:.1}%)", completed, if total_plans > 0 { completed as f32 / total_plans as f32 * 100.0 } else { 0.0 });
                    println!("🔄 In Progress: {} ({:.1}%)", in_progress, if total_plans > 0 { in_progress as f32 / total_plans as f32 * 100.0 } else { 0.0 });
                    println!("📋 Planned: {} ({:.1}%)", planned, if total_plans > 0 { planned as f32 / total_plans as f32 * 100.0 } else { 0.0 });
                } else {
                    println!("❌ Team '{}' not found", team_name);
                }
            } else {
                // Show workload for all teams
                println!("📊 Workload Summary\n");
                for (team_name, team_info) in &teams {
                    let empty_members = vec![];
                    let members = team_info.get("members").and_then(|m| m.as_array()).unwrap_or(&empty_members);
                    let team_plans: Vec<_> = plans.iter().filter(|p| {
                        p.owners.iter().any(|owner| members.iter().any(|m| m.as_str() == Some(owner)))
...
fn handle_analytics_command(cmd: AnalyticsCmd, root: &std::path::Path) -> Result<()> {
    let store = PlanStore::new(&root);
    let plans = store.load_all()?;
    
    match cmd {
        AnalyticsCmd::Blocked => {
            let blocked_plans: Vec<_> = plans.iter().filter(|p| p.status == PlanStatus::Blocked).collect();
            
            println!("🚫 Blocked Items ({})\n", blocked_plans.len());
            
            if blocked_plans.is_empty() {
                println!("✅ No blocked items found!");
            } else {
                for plan in blocked_plans {
                    println!("🔴 {} - {}", plan.id.red(), plan.title);
                    println!("   Type: {} | Priority: {}", plan.plan_type.as_str(), plan.priority.as_str());
                    if !plan.blocks.is_empty() {
                        println!("   Blocking: {}", plan.blocks.join(", "));
                    }
                    println!();
                }
                ...
                // Suggest actions
                println!("💡 Suggestions:");
                println!("   • Review blocked items in daily standup");
                println!("   • Identify dependencies and create action plans");
                println!("   • Consider escalation for high-priority blocked items");
            }
        }
        
        AnalyticsCmd::CycleHealth { cycle } => {
            println!("🏥 Cycle Health Analysis\n");
            
            let cycle_plans: Vec<_> = if let Some(cycle_name) = cycle {
                plans.iter().filter(|p| p.release.as_ref() == Some(&cycle_name)).collect()
            } else {
                plans.iter().collect()
            };
            
            let total = cycle_plans.len();
            let completed = cycle_plans.iter().filter(|p| p.status == PlanStatus::Done).count();
            let in_progress = cycle_plans.iter().filter(|p| p.status == PlanStatus::InProgress).count();
            let blocked = cycle_plans.iter().filter(|p| p.status == PlanStatus::Blocked).count();
            
            println!("📊 Overview:");
            println!("   Total items: {}", total);
            println!("   ✅ Completed: {} ({:.1}%)", completed, if total > 0 { completed as f32 / total as f32 * 100.0 } else { 0.0 });
            println!("   🔄 In Progress: {} ({:.1}%)", in_progress, if total > 0 { in_progress as f32 / total as f32 * 100.0 } else { 0.0 });
            println!("   🚫 Blocked: {} ({:.1}%)", blocked, if total > 0 { blocked as f32 / total as f32 * 100.0 } else { 0.0 });
            
            // Health score
            let health_score = if total > 0 {
                (completed as f32 + in_progress as f32 * 0.5) / total as f32 * 100.0 - blocked as f32 / total as f32 * 20.0
            } else {
                100.0
            };
            
            println!("\n🏥 Health Score: {:.1}%", health_score.max(0.0));
            
            if health_score >= 80.0 {
                println!("   Status: {} Excellent progress!", "🟢".green());
            } else if health_score >= 60.0 {
                println!("   Status: {} Good progress with room for improvement", "🟡".yellow());
            } else {
                println!("   Status: {} Needs attention - consider intervention", "🔴".red());
            }
        }
        
        AnalyticsCmd::TeamPerformance { team } => {
            // Load teams
            let teams_file = root.join(".rune").join("teams.json");
            if !teams_file.exists() {
                println!("❌ No teams configured. Use 'rune plan teams create' first.");
                return Ok(());
            }
            
            let teams: std::collections::HashMap<String, serde_json::Value> = {
                let content = std::fs::read_to_string(&teams_file)?;
                serde_json::from_str(&content)?
            };
            
            if let Some(team_info) = teams.get(&team) {
                let empty_members = vec![];
                let members = team_info.get("members").and_then(|m| m.as_array()).unwrap_or(&empty_members);
                let team_plans: Vec<_> = plans.iter().filter(|p| {
                    p.owners.iter().any(|owner| members.iter().any(|m| m.as_str() == Some(owner)))
                ...
                let total = team_plans.len();
                let completed = team_plans.iter().filter(|p| p.status == PlanStatus::Done).count();
                let avg_effort: f32 = team_plans.iter().filter_map(|p| p.effort).sum::<u32>() as f32 / team_plans.len().max(1) as f32;
                
                println!("📊 Metrics:");
                println!("   Total items: {}", total);
                println!("   Completion rate: {:.1}%", if total > 0 { completed as f32 / total as f32 * 100.0 } else { 0.0 });
                println!("   Average effort: {:.1} points", avg_effort);
                
                // Velocity (items completed per week - rough estimate)
                let completed_this_month = team_plans.iter().filter(|p| {
                    p.status == PlanStatus::Done && 
                    p.updated.signed_duration_since(chrono::Utc::now()).num_days().abs() <= 30
                }).count();
                
                println!("   Recent velocity: {} items/month", completed_this_month);
                
                println!("\n💡 Insights:");
                if completed as f32 / total.max(1) as f32 > 0.8 {
                    println!("   ✅ High completion rate - team is performing well");
                } else if completed as f32 / total.max(1) as f32 > 0.5 {
                    println!("   🟡 Moderate completion rate - consider process improvements");
                } else {
                    println!("   🔴 Low completion rate - may need support or workload adjustment");
                }
            } else {
                println!("❌ Team '{}' not found", team);
            }
        ...
        AnalyticsCmd::Dependencies { id } => {
            if let Ok(plan) = store.load(&id) {
                println!("🔗 Dependencies for: {} - {}\n", plan.id.cyan(), plan.title);
                
                // Show what this plan depends on
                if !plan.dependencies.is_empty() {
                    println!("📥 Depends on:");
                    for dep in &plan.dependencies {
                        if let Ok(dep_plan) = store.load(&dep) {
                            println!("   • {} - {} ({})", dep.cyan(), dep_plan.title, dep_plan.status.as_str());
                        } else {
                            println!("   • {} (not found)", dep.red());
                        }
                    }
                    println!();
                }
                
                // Show what depends on this plan
                let dependents: Vec<_> = plans.iter().filter(|p| p.dependencies.contains(&id)).collect();
                if !dependents.is_empty() {
                    println!("📤 Blocks:");
                    for dep in &dependents {
                        println!("   • {} - {} ({})", dep.id.cyan(), dep.title, dep.status.as_str());
                    }
                    println!();
                }
                
                if plan.dependencies.is_empty() && dependents.is_empty() {
                    println!("ℹ No dependencies found for this item.");
                }
            } else {
        ...
        AnalyticsCmd::Bottlenecks => {
            println!("🚦 Bottleneck Analysis\n");
            
            // Find items that block many others
            let mut blocking_count: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
            for plan in &plans {
                for dep in &plan.dependencies {
                    *blocking_count.entry(dep.clone()).or_insert(0) += 1;
                }
            }
            
            let mut bottlenecks: Vec<_> = blocking_count.iter().collect();
            ...
            if bottlenecks.is_empty() {
                ...
                for (plan_id, count) in bottlenecks.iter().take(5) {
                    if let Ok(plan) = store.load(plan_id) {
                        println!("   {} blocks {} items - {} ({})", 
                            plan_id.red(), 
                            count, 
                            plan.title, 
                            plan.status.as_str()
                ...
                println!("\n💡 Recommendations:");
                println!("   • Prioritize resolving top bottlenecks");
                println!("   • Consider breaking down complex items");
                println!("   • Add more resources to critical path items");
            }
        }
        
        AnalyticsCmd::Forecast { days } => {
            let forecast_days = days.unwrap_or(30);
            println!("🔮 Completion Forecast ({} days)\n", forecast_days);
            
            // Simple velocity-based forecast
            let completed_last_30_days = plans.iter().filter(|p| {
                p.status == PlanStatus::Done && 
                p.updated.signed_duration_since(chrono::Utc::now()).num_days().abs() <= 30
            }).count();
            
            let remaining_items = plans.iter().filter(|p| p.status != PlanStatus::Done).count();
            let daily_velocity = completed_last_30_days as f32 / 30.0;
            let estimated_days = if daily_velocity > 0.0 {
                remaining_items as f32 / daily_velocity
            } else {
                0.0
            };
            
            println!("📊 Current metrics:");
            println!("   Completed last 30 days: {}", completed_last_30_days);
            println!("   Daily velocity: {:.2} items/day", daily_velocity);
            println!("   Remaining items: {}", remaining_items);
            
            println!("\n🔮 Forecast:");
            if daily_velocity > 0.0 {
                println!("   Estimated completion: {:.0} days from now", estimated_days);
                let completion_date = chrono::Utc::now() + chrono::Duration::days(estimated_days as i64);
                println!("   Target date: {}", completion_date.format("%Y-%m-%d"));
                
                if estimated_days <= forecast_days as f32 {
                    println!("   Status: {} On track for completion!", "🟢".green());
                } else {
                    println!("   Status: {} May need additional resources", "🟡".yellow());
                }
            } else {
                println!("   Status: {} No recent velocity data available", "🔴".red());
            }
        }
        ...
        AnalyticsCmd::Risk => {
            ...
            // Check for blocked high-priority items
            let blocked_critical = plans.iter().filter(|p| 
                p.status == PlanStatus::Blocked && p.priority == Priority::Critical
            ).count();
            if blocked_critical > 0 {
                risks.push(format!("🔴 {} critical items blocked", blocked_critical));
            }
            
            // Check for overdue items (rough estimate)
            let long_in_progress = plans.iter().filter(|p| 
                p.status == PlanStatus::InProgress && 
                p.updated.signed_duration_since(chrono::Utc::now()).num_days().abs() > 14
            ).count();
            if long_in_progress > 0 {
                risks.push(format!("🟡 {} items in progress >14 days", long_in_progress));
            }
            
            // Check dependency chains
            let complex_dependencies = plans.iter().filter(|p| p.dependencies.len() > 3).count();
            if complex_dependencies > 0 {
                risks.push(format!("🟠 {} items with >3 dependencies", complex_dependencies));
            }
            
            if risks.is_empty() {
                println!("✅ No significant risks detected!");
                println!("\n🎯 Project appears to be on track.");
            } else {
                println!("⚠️ Identified risks:");
                for risk in &risks {
                    println!("   {}", risk);
                }
                
                println!("\n🛡️ Mitigation strategies:");
                println!("   • Daily standups to address blockers");
                println!("   • Regular dependency review");
                println!("   • Consider parallel work streams");
                println!("   • Escalate critical blockers immediately");
            }
        }
        
        AnalyticsCmd::Report { output } => {
            println!("📊 Generating Analytics Report...\n");
            
            let total_plans = plans.len();
            let completed = plans.iter().filter(|p| p.status == PlanStatus::Done).count();
            let in_progress = plans.iter().filter(|p| p.status == PlanStatus::InProgress).count();
            let blocked = plans.iter().filter(|p| p.status == PlanStatus::Blocked).count();
            ...
            let report = format!(
                ...
                total_plans,
                completed, if total_plans > 0 { completed as f32 / total_plans as f32 * 100.0 } else { 0.0 },
                in_progress, if total_plans > 0 { in_progress as f32 / total_plans as f32 * 100.0 } else { 0.0 },
                blocked, if total_plans > 0 { blocked as f32 / total_plans as f32 * 100.0 } else { 0.0 },
                plans.iter().filter(|p| p.status == PlanStatus::Planned).count(),
                plans.iter().filter(|p| p.status == PlanStatus::InProgress).count(),
                completed,
                blocked
            );
