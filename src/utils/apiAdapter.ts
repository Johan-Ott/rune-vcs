// API Adapter Layer - Provides interface for switching between mock and real Rune VCS API
// This utility makes it easy to switch from mock data to real Rune VCS backend integration
...
// Global API configuration for Rune VCS
let apiConfig: APIConfig = {
  useRealAPI: false, // Will auto-detect if Rune VCS API is available
  baseURL: (import.meta as any).env?.VITE_RUNE_API_URL || 'http://localhost:3000',
  runeVCSPath: (import.meta as any).env?.VITE_RUNE_VCS_PATH || '/usr/local/bin/rune',
  timeout: 10000,
  autoDetect: true
};
